...
class eLCS(BaseEstimator):
    ...
    def predict(self, X):
        ...
        for inst in range(instances):
            tstate = X[inst]
            state = np.array([])
            for i in range(tstate.size):
                state = np.append(state,MultiStateValue(tstate[i]))
